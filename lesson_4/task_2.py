...
def get_spec_list(max_value: int, max_len: int) -> list[int]: # input_list: list[int]
    ...
    """
    input_list = [randint(1, max_value) for _ in range(max_len + 1)]
    spec_list: list[int] = [el for idx, el in enumerate(input_list) if idx > 0 and input_list[idx] > input_list[idx - 1]]
    return spec_list
...
def main() -> None:
    ...
    #test_get_spec_list()
    #input_list: list[int] = generate_list(300)
    output_list: list[int] = get_spec_list(300, 10)#input_list)
